...
const services: readonly Service[] = (() => {
  const decode = Schema.decodeUnknownSync(ServiceFromRow)
  return [
    decode({
      id: SERVICE_TRIM_ID,
...
const providers: readonly Provider[] = (() => {
  const decode = Schema.decodeUnknownSync(ProviderFromRow)
  return [
    decode({
...
const resources: readonly Resource[] = (() => {
  const decode = Schema.decodeUnknownSync(ResourceFromRow)
  return [
    decode({
...
const closures: readonly Closure[] = (() => {
  const decode = Schema.decodeUnknownSync(ClosureFromRow)
  return [
    decode({
      id: id("clos", 1),
      date: "2026-12-31",
...
const document: readonly string[] = [
  ...
  "",
  ...services.map((s) => upsertOne("services", ServiceFromRow, s)),
  ...providers.map((p) => upsertOne("providers", ProviderFromRow, p)),
  ...resources.map((r) => upsertOne("resources", ResourceFromRow, r)),
  ...businessHours.map((bh) => upsertOne("business_hours", BusinessHoursFromRow, bh)),
  ...closures.map((c) => upsertOne("closures", ClosureFromRow, c)),
  ...providerAbsences.map((a) => upsertOne("provider_absences", ProviderAbsenceFromRow, a)),
  "",
]
